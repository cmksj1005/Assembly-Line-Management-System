// Name: Shinjo Kang
// Seneca Student ID: 118135227
// Seneca email: skang79@myseneca.ca
// Date of completion: 08-05-2023
//
...
namespace sdds {
		...
		if (m_cntItem > 0) {
			for (size_t i = 0; i < m_cntItem; i++) {
				if (m_lstItem[i]->m_itemName == station.getItemName()) {
					...
					else { // *** be careful when I do MS 3. I think this is not for MS 2 so I don't check whether this part is right
	...
	void CustomerOrder::display(std::ostream& os) const {
		...
		for (size_t i = 0; i < m_cntItem; i++) {
			os << "[" << std::setfill('0') << std::setw(6) << m_lstItem[i]->m_serialNumber << "] " << std::left << std::setfill(' ') << std::setw(m_widthField) << m_lstItem[i]->m_itemName << " - ";
			if (m_lstItem[i]->m_isFilled) {
				os << "FILLED" << endl;
			}
			else {
